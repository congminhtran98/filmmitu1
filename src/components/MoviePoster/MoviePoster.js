...
function MoviePoster({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let fetchDateType = ``;
      type === 'movies'
        ? (fetchDateType = movieDetail)
        : (fetchDateType = tvDetail);

      var request = await axios.get(fetchDateType);
      let trailerIndex = request.data.videos.results.findIndex(
        (v) => v.type === 'Trailer'
      );
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${baseUrl}${
              isHover ? movie.backdrop_path : movie.poster_path
            })`,
          }}
        >
